<script>
  ...
  onMount(() => {
        // reveal the loader text 
        const revealLoader = [loaderText, {opacity: 1}, {duration: 0}]

        // split the hero text + animation
        const splitLoader = new SplitType(loaderText, {lineClass: 'overflow-hidden -my-16'})
        const splitLoaderIn = [splitLoader.words, {y: ['100%', '0%']}, {delay: stagger(0.1), duration: 0.8}]
        const splitLoaderOut = [splitLoader.words, {y: ['-100%']}, {delay: stagger(0.1), duration: 0.8, at: "+0.1"}]
        ...
        const animateHero = [splitHero.words, {y: ['100%', '0%']}, {delay: stagger(0.1), duration: 1, easing: [0.62, -0.01, 0.37, 0.99]}]
        ...
        timeline(
          [revealLoader, splitLoaderIn, splitLoaderOut, animateLoaderBG, animateHero, animateBG, animateLeft, animateRight, animateCTA],
          {
            defaultOptions: {
              ease: [0.52, 0.05, 0.22, 0.95]
            }
          }
          )
    })
...
<div class="h-screen w-full flex items-center justify-center bg-primary-400 relative">
  <!-- Hero text -->
  <h1 bind:this="{heroText}" class="font-primary text-huge text-center text-light-100 z-20">Welcome<br>to the<br>Jungle</h1>

  <!-- Background image -->
  <div class="absolute w-full h-full">
      <div class="h-full w-full absolute bg-hero-fade"></div>
      <img bind:this="{bg}" src="{forestBG}" alt="" class="blur-sm h-full w-full bg-cover">
  </div>
  ...
  <!-- CTA Button -->
  <button bind:this="{ctaButton}" class="absolute -bottom-14 rounded-full p-4 border border-light-100">
  ...
  <div class="z-50 fixed h-screen w-screen flex items-center justify-center">
    <div bind:this="{loaderBG}" class="absolute bg-primary-400 w-full h-full"></div>
    <span bind:this="{loaderText}" class="font-primary text-3xl text-light-100 relative opacity-0">Jungle Cat</span>
  </div>
</div>